...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	 ...
	 */
	/**工厂中的LifecycleProcessor bean的名称。
	...
	/**此上下文使用的环境。包括系统环境变量等，还可以自己实现，然后注入值，比如web就有一些自定义的 */
	@Nullable
	private ConfigurableEnvironment environment;

	/** 在刷新时应用BeanFactoryPostProcessors。 */
	...
	/**这个context使用的ResourcePatternResolver. */
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				// 允许在上下文子类中对bean工厂进行后处理。  对工厂进行一些修改操作
				...
				/*注册BeanPostProcessor 用于扩展或者修改bean，伴随bean的创建的生命周期，在bean实例化和初始化的时候调用，上面执行的是BeanFactoryPostProcessor*/
				/*比如AOP都是使用bean的后置处理器来操作的。注册bean后置处理器*/
				registerBeanPostProcessors(beanFactory);

				// 注册 MessageSource单例对象放入容器中 要来干嘛？
				initMessageSource();
	...
	protected void prepareRefresh() {
		...
		//在上下文环境中初始化所有占位符属性源。用于替换#{}吧  准备Environment对象：填充PropertySources用于后面解析文件或者其他地方的#{}
		initPropertySources();

		// Validate that all properties marked as required are resolvable:
		// see ConfigurablePropertyResolver#setRequiredProperties
		/*验证属性必须要的属性是否都有了，并且加载系统的环境变量jvm参数到容器中（放入Environment对象中）*/
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 实例化所有剩余的(非lazy-init)单例。---实例化单例bean
